...
folder_id = fopen('基本数据/Katz_50_average_var_result.txt','w+');
for foler_index=1:30
    for index1 = 1:15
       fid = fopen(['基本数据/data' num2str(foler_index) '/' file{index1,6} '/CN_result.txt'],'w+');
       r_reindex = asstcoe(['基本数据/data' num2str(1) '/' file{index1,6} '/reindex.txt']);
       ...
       total_data = load(['基本数据/data' num2str(1) '/' file{index1,6} '/reindex.txt'],'-ascii');
       ...
       sn = zeros(max_label,max_label);%邻接矩阵
       ...
       disp(['开始第', num2str(index1),'个数据处理!']);
        ...
        sn = sn-diag(diag(sn));
        S =inv(sparse(eye(size(sn,1)))-0.01*sn); %矩阵对应节点对公共邻居数
        S(isnan(S)) = 0;
        S(isinf(S)) = 0;
        S=S-sparse(eye(size(sn,1)));
        S(logical(sn)) = 0; %除去原来相连的点对
        S = S-diag(diag(S));
        S = triu(S, 0); %去掉对称的重复
		
        test_line_matrix = load(['基本数据/data' num2str(foler_index) '/' file{index1,6} '/testing.txt'],'-ascii');
