...
folder_id = fopen('data/LP论文2_100_average_var_resulte0.9.txt','w+');
for foler_index=1:30
    for index1 = 1:13
       fid = fopen(['data/data' num2str(foler_index) '/' file{index1,6} '/LP_result.txt'],'w+');
       ...
       for row_index = 1:rows %得到训练集的系数矩阵sn？？？？
       ...
       disp(['开始第', num2str(index1),'个数据处理!']);
        sn = FormNet(training_set);%构建训练集的邻接矩阵
        sn = full(sn); %full将稀疏矩阵转换为全矩阵
        sn = sn-diag(diag(sn));%对角元置位0
        ...
        e = 0.9; 
	    S = S1+e*S2;
        S(logical(sn)) = 0; %函数logical()实现数据从数值型到逻辑型的转换--做下标   将训练集矩阵中原有连线在相似性矩阵对应位置置为0
        S = S-diag(diag(S));
        S = triu(S, 0); %去上三角
...
for data_index=1:13
    ...
    fprintf(folder_id, '%f %f %f %f %f\n',sum(precision_matrix{data_index})/length(precision_matrix{data_index}), sum( AUC_matrix{data_index})/length( AUC_matrix{data_index}), variance_precision, sum(hit_matrix{data_index})/length(hit_matrix{data_index}), variance_hit);
