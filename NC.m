...
file{6,5} = 'data/Jazz/reindex.txt';
file{6,6} = 'Jazz';

file{7,1} = 'data/email/training.txt';
file{7,2} = 'data/email/testing.txt';
file{7,3} = 'data/email/email_precision.txt';
file{7,4} = 'data/email/email_AUC.txt';
file{7,5} = 'data/email/reindex.txt';
file{7,6} = 'email';

file{8,1} = 'data/Slavko/training.txt';
file{8,2} = 'data/Slavko/testing.txt';
file{8,3} = 'data/Slavko/Slavko_precision.txt';
file{8,4} = 'data/Slavko/Slavko_AUC.txt';
file{8,5} = 'data/Slavko/reindex.txt';
file{8,6} = 'Slavko';

file{9,1} = 'data/ucsocial/training.txt';
file{9,2} = 'data/ucsocial/testing.txt';
file{9,3} = 'data/ucsocial/ucsocial_precision.txt';
file{9,4} = 'data/ucsocial/ucsocial_AUC.txt';
file{9,5} = 'data/ucsocial/reindex.txt';
file{9,6} = 'ucsocial';

file{10,1} = 'data/Infec/training.txt';
file{10,2} = 'data/Infec/testing.txt';
file{10,3} = 'data/Infec/Infec_precision.txt';
file{10,4} = 'data/Infec/Infec_AUC.txt';
file{10,5} = 'data/Infec/reindex.txt';
file{10,6} = 'Infec';

file{11,1} = 'data/EuroSiS/training.txt';
file{11,2} = 'data/EuroSiS/testing.txt';
file{11,3} = 'data/EuroSiS/EuroSiS_precision.txt';
file{11,4} = 'data/EuroSiS/EuroSiS_AUC.txt';
file{11,5} = 'data/EuroSiS/reindex.txt';
file{11,6} = 'EuroSiS';

file{12,1} = 'data/CE/training.txt';
file{12,2} = 'data/CE/testing.txt';
file{12,3} = 'data/CE/CE_precision.txt';
file{12,4} = 'data/CE/CE_AUC.txt';
file{12,5} = 'data/CE/reindex.txt';
file{12,6} = 'CE';
% file{13,1} = 'data/Router/training.txt';
% file{13,2} = 'data/Router/testing.txt';
% file{13,3} = 'data/Router/Router_precision.txt';
% file{13,4} = 'data/Router/Router_AUC.txt';
% file{13,5} = 'data/Router/reindex.txt';
% file{13,6} = 'Router';
% 
% file{14,1} = 'data/King Jame/training.txt';
% file{14,2} = 'data/King Jame/testing.txt';
% file{14,3} = 'data/King Jame/King Jame_precision.txt';
% file{14,4} = 'data/King Jame/King Jame_AUC.txt';
% file{14,5} = 'data/King Jame/reindex.txt';
% file{14,6} = 'King Jame';

precision_matrix = cell(1,12);
AUC_matrix = cell(1,12);
hit_matrix = cell(1,12);
RMSE_matrix = cell(1,12);

for r=2:15
random_walk_steps = r;
folder_id = fopen(['data/NC换h_var_average_result_walk=' num2str(random_walk_steps ) ' .txt'],'w+');
...
for foler_index=1:30
    for index= 1:12  %数据集
        fid = fopen(['data/data' num2str(foler_index) '/' file{index,6} '/NC_result.txt'],'w+');
...
%max_fir=(A_matrix.*base);
%max_sec=(max(max_fir))';
%max_thi=repmat(max_sec,1,max_label);

H_index=repmat(h,1,max_label);%h指数矩阵

%ormalization_d_matrix = A_matrix*(base/max(node_degree_vector));%max_thi
normalization_h_matrix = (A_matrix*(H_index./max(h)));
%dh = (normalization_d_matrix.*normalization_h_matrix).^(1/2);
dh = normalization_h_matrix;
