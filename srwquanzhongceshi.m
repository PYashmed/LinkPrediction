...
% file{1,1} = 'data/CS phd/training.txt';
% file{1,2} = 'data/CS phd/testing.txt';
% file{1,3} = 'data/CS phd/CS phd_precision.txt';
% file{1,4} = 'data/CS phd/CS phd_AUC.txt';
% file{1,5} = 'data/CS phd/reindex.txt';
% file{1,6} = 'CS phd';

% file{1,1} = 'data/EVA/training.txt';
% file{1,2} = 'data/EVA/testing.txt';
% file{1,3} = 'data/EVA/CS phd_precision.txt';
% file{1,4} = 'data/EVA/CS phd_AUC.txt';
% file{1,5} = 'data/EVA/reindex.txt';
% file{1,6} = 'EVA';

precision_matrix = cell(1,15);
AUC_matrix = cell(1,15);
hit_matrix = cell(1,15);
RMSE_matrix = cell(1,15);

%folder_id = fopen('data/SRW_average_result.txt','w+');
random_walk_steps =4;
folder_id = fopen(['data/USAIR=' num2str(random_walk_steps ) ' .txt'],'w+');
...
for foler_index=1:30
        ...
        fid = fopen(['data/data' num2str(foler_index) '/' file{index,6} '/SRW_result.txt'],'w+');
        ...
        %SRW算法开始  get degree vectors 
        node_degree_vector = sum(A_matrix,2);% degree column vector
%         kk=node_degree_vector./(max_label-1);
%         mm=log10(kk);
%         mm(isnan(mm)) = 0;
%         mm(isinf(mm)) = 0;
%         ME=-kk.*(A_matrix*mm);
        %get transition probabilty matrix
        P = A_matrix./(repmat(node_degree_vector, 1, max_label));%马尔科夫概率矩阵
       % P =(A_matrix.*repmat(node_degree_vector, 1, max_label))'./(A_matrix*repmat(node_degree_vector, 1, max_label));
       
        P(isnan(P)) = 0;
        P(isinf(P)) = 0;
        %random walk loop
        Simi_matrix = zeros(max_label);
        Pi_matrix = eye(max_label);%生成单位矩阵
        
        mm=log10(node_degree_vector);
        mm(isnan(mm)) = 0;
        mm(isinf(mm)) = 0;
        ME=(node_degree_vector).*(A_matrix*mm);

%         A_Size = size(A_matrix); 
%         M = zeros(A_Size(1), 1); 
%         %  计算节点的度值 
%         %b = sum(A, 2); 
%         %计算各节点的邻接度 
%         for i=1:A_Size(1) 
%         M(i) = sum(node_degree_vector(find(A_matrix(i,:)))); 
%         end 
%         %  计算节点概率与邻居节点概率 
%         pi ={}; 
%         pj = {}; 
%         for i=1:A_Size(1) 
%         tmp =   M(find(A_matrix(i,:))); 
%         pi(i) = {node_degree_vector(i)./tmp}; 
%         pj(i) = {node_degree_vector(find(A_matrix(i,:)))./tmp}; 
%         end 
%         %  计算节点的信息熵 
...
%         mm=log10(node_degree_vector./(2*rows));
%         mm(isnan(mm)) = 0;
%         mm(isinf(mm)) = 0; 
%         ME=-(node_degree_vector./(2*rows)).*(A_matrix*mm);

%       DD=node_degree_vector.*(A_matrix*node_degree_vector);
%       LL=DD.^(1/2);
        ...
        for step=1:random_walk_steps
            ...
            if step>1
                x_matrix =repmat(node_degree_vector,1,max_label);
...
%     fprintf(folder_id, '%s: average_precision=%f, average_AUC=%f, average_hit=%f, average_RMSE=%f\n', file{data_index,6}, sum(precision_matrix{data_index})/length(precision_matrix{data_index}), sum( AUC_matrix{data_index})/length( AUC_matrix{data_index}), sum(hit_matrix{data_index})/length(hit_matrix{data_index}), sum(RMSE_matrix{data_index})/length(RMSE_matrix{data_index}));
    fprintf(folder_id, '%f %f\n',sum(precision_matrix{index})/length(precision_matrix{index}), sum( AUC_matrix{index})/length( AUC_matrix{index}));
