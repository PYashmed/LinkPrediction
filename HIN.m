...
for r=2:4
random_walk_steps = r;
folder_id = fopen(['data/论文里coreSRW__var_average_result_walk=' num2str(random_walk_steps ) ' .txt'],'w+');
...
for foler_index=1:30
    for index= 1:13  %数据集
        fid = fopen(['data/data' num2str(foler_index) '/' file{index,6} '/HIN_result.txt'],'w+');
...
%get transition probabilty matr
P = A_matrix./ core_base;
...
%{
min_degree=min(node_degree_vector)-1;
max_degree=max(node_degree_vector)-1;
base=(base-1-min_degree)./(max_degree-min_degree);
min_h=min(node_degree_vector)-1;
max_h=max(node_degree_vector)-1;
H_index=(H_index-1-min_h)./(max_h-min_h);
%}

average_core_matrix = (A_matrix*core_base)./(core);
% average_d_matrix = (A_matrix*base)/(node_degree_vector.');%平均邻居度矩阵
average_h_matrix = (A_matrix*H_index)./(core);%平均h指数矩阵

average_core_matrix(isnan(average_core_matrix)) = 0;
% average_d_matrix(isinf(average_d_matrix)) = 0;
average_h_matrix (isnan(average_h_matrix )) = 0;
% average_h_matrix (isinf(average_h_matrix )) = 0;
% average_core_matrix =repmat(average_core_matrix,1,max_label);
% average_h_matrix =repmat(average_h_matrix,1,max_label);

dh = (average_core_matrix.*average_h_matrix).^(1/2);
